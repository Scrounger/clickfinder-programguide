...
#Region "Skin Controls"
        ...
        <SkinControlAttribute(45)> Protected DetailsFanArt As GUIImage = Nothing

        <SkinControlAttribute(86)> Protected btnPlay As GUIButtonControl = Nothing
...
#Region "GUI Events"
        ...
        Protected Overrides Sub OnClicked(ByVal controlId As Integer, _
            ...
            If control Is btnPlay Then


                'Dim NewItem As New PlayListItem()
                'NewItem.FileName = _CurrentFileName
                ...
                g_Player.Play(_CurrentFileName)
                ...
                ''foundRow = _CurrentDatatable.Select("SendungID = " & _CurrentDetailsSendungId)

                'Dim DatatableIndex As Integer = _CurrentDatatable.Rows.IndexOf(_CurrentDatatable.Rows.Find(_CurrentDetailsSendungPos))

                ''ShowItemDetails(_CurrentDatatable.Rows(DatatableIndex + 1).Item("Sendungen.Pos"),
                'Try
                '    MsgBox(
                'Catch ex As Exception
                '    MsgBox(ex.Message)
...
#Region "Click Events"
        ...
        Private Sub Button_ViewChannel()
            ...
            Log.Debug("")
            Log.Debug("Clickfinder ProgramGuide: [Button_ViewChannel]: Clicked")
            Log.Debug("")

            Try
                'Channel über KanalName starten
                Dim sb As New SqlBuilder(StatementType.[Select], GetType(Channel))
                sb.AddConstraint([Operator].Equals, "displayName", DetailsKanal.Label)

                Dim stmt As SqlStatement = sb.GetStatement(True)
                Dim _Result As IList = ObjectFactory.GetCollection(GetType(Channel), stmt.Execute())

                Dim _Channel As Channel = _Result.Item(0)
        ...
        Private Sub Button_Record()
            ...
            Try
                ...
                Log.Debug("Clickfinder ProgramGuide: [Button_Record]: LoadTVProgramInfo(" & _ClickfinderDB(0).TvServer_idchannel & "," & _ClickfinderDB(0).Beginn & "," & _ClickfinderDB(0).Ende & "," & _ClickfinderDB(0).Titel & ")")
                LoadTVProgramInfo(_ClickfinderDB(0).TvServer_idchannel, _ClickfinderDB(0).Beginn, _ClickfinderDB(0).Ende, _ClickfinderDB(0).Titel)
        ...
        Private Sub Button_Remember()
            Try
                ...
                Log.Debug("Clickfinder ProgramGuide: [Button_Remember]: SetNotify(" & _ClickfinderDB(0).TvServer_idchannel & "," & _ClickfinderDB(0).Beginn & "," & _ClickfinderDB(0).Ende & "," & _ClickfinderDB(0).Titel & ")")
                SetNotify(_ClickfinderDB(0).TvServer_idchannel, _ClickfinderDB(0).Beginn, _ClickfinderDB(0).Ende, _ClickfinderDB(0).Titel)
...
#Region "Show Events"
        ...
        Private Sub ShowSelectedCategorieItems()
            ...
            Try
                ...
                If MPSettingRead("MPTVSeries", "enable") = "true" And _CurrentCategorie = "Serien" Then
                            ...
                            Select Case _ClickfinderDB(i).Titel & _ClickfinderDB(i).Originaltitel
                                ...
                                Case Else
                                    ...
                                    If _useRatingTvLogos = "true" Then
                                        _TvLogo = Config.GetFile(Config.Dir.Thumbs, "ClickfinderPG\tv\logos\") & _ClickfinderDB(i).TvServer_displayName & "_" & _ClickfinderDB(i).Bewertung & ".png"
                                    Else
                                        _TvLogo = Config.GetFile(Config.Dir.Thumbs, "tv\logos\" & _ClickfinderDB(i).TvServer_displayName & ".png")
                                    End If
                                            ...
                                            If MPSettingRead("MPTVSeries", "enable") = "true" Then
                                                If _TVSeriesDB.SeriesFound(_ClickfinderDB(i).Titel) = True Then
                                                    ...
                                                    If _TVSeriesDB.EpisodeFound(Sendung.EpisodeName) = True Then
                                                        ...
                                                        Else
                                                            If Not _TVSeriesDB.SeasonIndex = 0 Or Not _TVSeriesDB.EpisodeIndex = 0 Then
            ...
            Try
                ...
                For i As Integer = 0 To _ClickfinderDB.Count - 1
                    ...
                    If _ClickfinderDB(i).Dauer >= _SettingMinTime Then
                        ...
                        Dim _Rating As Integer = _ClickfinderDB(i).Rating
                        ...
                        If ProgramFoundinTvDb(_ClickfinderDB(i).Titel, _ClickfinderDB(i).TvServer_idchannel, _ClickfinderDB(i).Beginn, _ClickfinderDB(i).Ende, _ClickfinderDB(i).SenderKennung) = True Then
                            ...
                            If Not _lastTitel = _ClickfinderDB(i).Titel & _ClickfinderDB(i).Originaltitel And _RespectInFavGroup = True And ChannelFoundInFavGroup(_ClickfinderDB(i).TvServer_idchannel) = True Then
                                ...
                                Else
                                    ...
                                    Log.Debug("Clickfinder ProgramGuide: [FillTipps: " & _TippsCounter & "]: ClickfinderID: " & _ClickfinderDB(i).SendungID)
                                    Log.Debug("Clickfinder ProgramGuide: [FillTipps: " & _TippsCounter & "]: Titel: " & _ClickfinderDB(i).Titel)
                                    Log.Debug("Clickfinder ProgramGuide: [FillTipps: " & _TippsCounter & "]: orgTitel: " & _ClickfinderDB(i).Originaltitel)
                                    Log.Debug("Clickfinder ProgramGuide: [FillTipps: " & _TippsCounter & "]: channel: " & _ClickfinderDB(i).TvServer_displayName)
                                    Select Case _CurrentCategorie
                                        Case Is = "Serien"
                                                ...
                                                If _TVSeriesDB.SeriesFound(_ClickfinderDB(i).Titel) = True Then
                                                    ...
                                                    Select Case MPSettingRead("MPTVSeries", "TippsImage")
                                                        Case Is = "SerienPoster"
                                                            _BildDatei = _TVSeriesDB.SeriesPosterBannerFileName
                                                        Case Is = "SerienBanner"
                                                            _BildDatei = _TVSeriesDB.CurrentBannerFileName
                                                        Case Is = "SerienFanArt"
                                                            _BildDatei = _TVSeriesDB.FanArt
                                                    End Select

                                                    If _TVSeriesDB.EpisodeFound(Sendung.EpisodeName) = True Then

                                                        _Rating = _TVSeriesDB.EpisodeRating
                                    ...
                                    _TippClickfinderSendungChannelName(_TippsCounter) = _ClickfinderDB(i).TvServer_displayName
                                    _TippClickfinderSendungTitel(_TippsCounter) = _ClickfinderDB(i).Titel & _ClickfinderDB(i).Originaltitel
                                    FillTipps(_TippsCounter, Sendung.Title, _BildDatei, _ClickfinderDB(i).TvServer_displayName, _ClickfinderDB(i).Beginn, _
                                              _ClickfinderDB(i).Ende, _ClickfinderDB(i).Genre, _ClickfinderDB(i).Bewertungen, _ClickfinderDB(i).Kurzkritik, _
                                              "ClickfinderPG_R" & CStr(_ClickfinderDB(i).Bewertung) & ".png", _ClickfinderDB(i).Originaltitel & _EpisodeExist, _SeriesNum, _EpisodeNum, _ClickfinderDB(i).Bewertung, _Rating)
        ...
        Private Sub ShowItemDetails(ByVal ClickfinderSendungID As String, ByVal ChannelName As String)
            ...
            For i As Integer = 0 To _ClickfinderDB.Count - 1
                Try
                    Dim _Rating As Integer = _ClickfinderDB(i).Rating
                    Dim _Describtion As String = ""
                    Dim _BildDatei As String = ""
                    Dim _Regie As String = ""
                    ...
                    If ProgramFoundinTvDb(_ClickfinderDB(i).Titel, _ClickfinderDB(i).TvServer_idchannel, _ClickfinderDB(i).Beginn, _ClickfinderDB(i).Ende, _ClickfinderDB(i).SenderKennung) = True Then
                            ...
                            Case Is = "Serien"
                                ...
                                If MPSettingRead("MPTVSeries", "enable") = "true" Then
                                    If _TVSeriesDB.SeriesFound(_ClickfinderDB(i).Titel) = True Then
                                        ...
                                        If _TVSeriesDB.FanArt IsNot "" Then
                                            DetailsFanArt.Visible = True
                                            DetailsFanArt.FileName = _TVSeriesDB.FanArt
                                        End If

                                        If _TVSeriesDB.EpisodeFound(Sendung.EpisodeName) = True Then

                                            _Rating = _TVSeriesDB.EpisodeRating
                                            _CurrentFileName = _TVSeriesDB.EpisodeFilename

                                            If MPSettingRead("MPTVSeries", "DetailImage") = "Thumb" Then
                                                _BildDatei = _TVSeriesDB.thumbFilename
                                                'MsgBox(_BildDatei)
                                            End If
                                            If MPSettingRead("MPTVSeries", "ShowDescribtion") = "true" Then
                                                _Describtion = _TVSeriesDB.Summary
                                                _Regie = _TVSeriesDB.Director & " (Drehbuch: " & _TVSeriesDB.Writer & ")"
                                            End If
                        ...
                        DetailsTitel.Label = _ClickfinderDB(i).Titel & _EpisodeExist
                        ...
                        If Not _SeriesNum = "" Or Not _EpisodeNum = "" Then
                            DetailsOrgTitel.Label = _ClickfinderDB(i).Originaltitel & " (Staffel " & _SeriesNum & ", Episode " & _EpisodeNum & ")"
                        Else
                            DetailsOrgTitel.Label = _ClickfinderDB(i).Originaltitel
                        End If


                        If _Describtion = "" Then
                            If Not _ClickfinderDB(i).Beschreibung = "" Then
                                If _ClickfinderDB(i).KurzBeschreibung = "" Or _ClickfinderDB(i).KurzBeschreibung = "HDTV" Then
                                    _Describtion = Replace(_ClickfinderDB(i).Beschreibung, "<br><br>", vbNewLine)
                                Else
                                    _Describtion = Replace(_ClickfinderDB(i).KurzBeschreibung, "HDTV,", "") & vbNewLine & vbNewLine & Replace(_ClickfinderDB(i).Beschreibung, "<br><br>", vbNewLine)
                                End If
                            End If
                        End If
                        DetailsBeschreibung.Label = _Describtion

                        If _Regie = "" Then
                            _Regie = _ClickfinderDB(i).Regie
                        End If
                        DetailsRegie.Label = _Regie
                        ...
                        ElseIf InStr(_BildDatei, "MPTVSeriesBanners") Or InStr(_BildDatei, "Fan Art") Then
                            DetailsImage.KeepAspectRatio = True
                            DetailsImage.Centered = True
                        Else
            ...
            Try
                ...
                If _BewertungStr = "" Then
                    ...
                    Else
                        If Not _SeriesNum = "" Or Not _EpisodeNum = "" Then
                            _channelName = _Genre
                            _Genre = "Staffel " & _SeriesNum & ", Episode " & _EpisodeNum
                        End If
...
#Region "TV Movie and TVServer Database Function"

        Private Function ProgramFoundinTvDb(ByVal _Titel As String, ByVal _idChannel As Integer, ByVal _StartZeit As Date, ByVal _EndZeit As Date, ByVal SenderKennung As String) As Boolean
            Try

                Dim sb As New SqlBuilder(StatementType.Select, GetType(Program))
                sb.AddConstraint([Operator].Equals, "Title", _Titel)
                sb.AddConstraint([Operator].Equals, "idChannel", _idChannel)
                sb.AddConstraint([Operator].Equals, "startTime", _StartZeit)
                sb.AddConstraint([Operator].Equals, "endTime", _EndZeit)
                'sb.AddOrderByField(False, "stationName")
                Dim stmt As SqlStatement = sb.GetStatement(True)
                Dim _Result As IList = ObjectFactory.GetCollection(GetType(Program), stmt.Execute())
        ...
        Private Function ChannelFoundInFavGroup(ByVal _idchannel As Integer) As Boolean
            Try
                Dim _idGroup As Integer = CInt(MPSettingRead("config", "ChannelGroupID"))

                Dim sb As New SqlBuilder(StatementType.Select, GetType(GroupMap))
                sb.AddConstraint([Operator].Equals, "idChannel", _idchannel)
                sb.AddConstraint([Operator].Equals, "idGroup", _idGroup)
                'sb.AddOrderByField(False, "stationName")
                Dim stmt As SqlStatement = sb.GetStatement(True)
                Dim _Result As IList = ObjectFactory.GetCollection(GetType(GroupMap), stmt.Execute())
        ...
        Private Sub CreateClickfinderRatingTable()
            ...
            Dim _RatingField As New ClickfinderDB(Replace(SQLQueryAccess(_TestStartZeit, _TestEndZeit, "AND Bewertung = 3 AND Rating >= 1 AND KzFilm = true"), "*", "DISTINCT Rating"))

            'Prüfen ob Rating für die kommenden Stunden existiert - ClickfinderDB Update erkennen
            If _RatingField.Count <= 1 Then '<= 5
